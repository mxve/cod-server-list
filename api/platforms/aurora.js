...
async function main() {
    masters.H1 = new DPMaster({ host: config.dpmaster.aurora.master.host, port: config.dpmaster.aurora.master.port }, 20, 50, 'H1', config.dpmaster.aurora.games.h1.protocol);
    masters.IW7 = new DPMaster({ host: config.dpmaster.aurora.master.host, port: config.dpmaster.aurora.master.port }, 20, 50, 'IW7', config.dpmaster.aurora.games.iw7.protocol);
}
main();
...
async function getServers() {
    const h1 = await formatServers('H1');
    const iw7 = await formatServers('IW7');
    ...
    return {
        servers: {
            ...
            iw7,
            get all() {
                const servers = [].concat(this.h1, this.iw7)
                servers.sort((a, b) => {
